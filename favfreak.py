...
fingeRes = urlopen('https://raw.githubusercontent.com/alanEG/FavFreak/master/finger.json')
fingerprint = json.loads(fingeRes.read())
    ...
    |       |     |   \/   |       |    \u001b[31m\_ |______\u001b[0m\u001b[32m |     | |    \_
        ...
        for i in a.keys():
            if str(i) in fingerprint.keys():
                print("\u001b[31m["+fingerprint[str(i)]+"] \u001b[0m" + str(i) + " - count : " + str(len(a[i])))
               # print('\n'.join(a[i][:-12]))
                if len(a[i]) > 0:
                    for k in a[i]:
                        print("     " + k[:-12])
        ...
        if args.output:
            for i in a.keys():
                ...
                with open(filename.replace('-', '_'), "a") as f:
